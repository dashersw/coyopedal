...
static bool IRAM_ATTR urb_check_args(urb_t *urb)
{
...
static bool IRAM_ATTR transfer_check_usb_compliance(usb_transfer_t *transfer, usb_transfer_type_t type, unsigned int mps, bool is_in)
{
...
static esp_err_t endpoint_alloc(device_t *dev_obj, const usb_ep_desc_t *ep_desc, usbh_ep_config_t *ep_config, endpoint_t **ep_obj_ret)
{
    ...
    ep_obj = heap_caps_calloc(1, sizeof(endpoint_t), USB_HOST_OBJ_CAPS);
    if (ep_obj == NULL) {
...
{
    device_t *dev_obj = heap_caps_calloc(1, sizeof(device_t), USB_HOST_OBJ_CAPS);
    if (dev_obj == NULL) {
        return ESP_ERR_NO_MEM;
    }
...
// Runs inside the HCD interrupt for every completed isochronous URB; see the
// note on intr_hdlr_main in hcd_dwc.c for why it is in IRAM.
static bool IRAM_ATTR epN_pipe_callback(hcd_pipe_handle_t pipe_hdl, hcd_pipe_event_t pipe_event, void *user_arg, bool in_isr)
{
...
esp_err_t usbh_install(const usbh_config_t *usbh_config)
{
    ...
    esp_err_t ret;
    usbh_t *usbh_obj = heap_caps_calloc(1, sizeof(usbh_t), USB_HOST_OBJ_CAPS);
...
esp_err_t usbh_dev_set_desc(usb_device_handle_t dev_hdl, const usb_device_desc_t *device_desc)
{
    ...
    new_desc = heap_caps_malloc(sizeof(usb_device_desc_t), USB_HOST_OBJ_CAPS);
    if (new_desc == NULL) {
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t usbh_dev_set_config_desc(usb_device_handle_t dev_hdl, const usb_config_desc_t *config_desc_full)
{
    ...
    new_desc = heap_caps_malloc(config_desc_full->wTotalLength, USB_HOST_OBJ_CAPS);
    if (new_desc == NULL) {
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t usbh_dev_set_str_desc(usb_device_handle_t dev_hdl, const usb_str_desc_t *str_desc, int select)
{
    ...
    new_desc = heap_caps_malloc(str_desc->bLength, USB_HOST_OBJ_CAPS);
    if (new_desc == NULL) {
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t IRAM_ATTR usbh_ep_get_handle(usb_device_handle_t dev_hdl, uint8_t bEndpointAddress, usbh_ep_handle_t *ep_hdl_ret)
{
...
void *IRAM_ATTR usbh_ep_get_context(usbh_ep_handle_t ep_hdl)
{
    assert(ep_hdl);
    endpoint_t *ep_obj = (endpoint_t *)ep_hdl;
...
esp_err_t IRAM_ATTR usbh_ep_enqueue_urb(usbh_ep_handle_t ep_hdl, urb_t *urb)
{
...
esp_err_t IRAM_ATTR usbh_ep_resubmit_urb(usbh_ep_handle_t ep_hdl, urb_t *urb)
{
    endpoint_t *ep_obj = (endpoint_t *)ep_hdl;
    return hcd_urb_enqueue(ep_obj->constant.pipe_hdl, urb);
}

esp_err_t IRAM_ATTR usbh_ep_dequeue_urb(usbh_ep_handle_t ep_hdl, urb_t **urb_ret)
{
