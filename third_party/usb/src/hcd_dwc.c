...
#ifndef COYOPEDAL_USB_XFER_LIST_ISOC_MARGIN
#define COYOPEDAL_USB_XFER_LIST_ISOC_MARGIN        3
#endif
#define XFER_LIST_ISOC_MARGIN                   COYOPEDAL_USB_XFER_LIST_ISOC_MARGIN

#if CONFIG_IDF_TARGET_ESP32S3
// The HD X has three isochronous endpoints (capture, playback and feedback),
// and the HCD keeps two descriptor lists per endpoint.  Reserve those six
// 512-byte-aligned DMA blocks at link time.  Late aligned heap allocation is
// unreliable once the A2-Full model, Wi-Fi and effects have fragmented the
// S3's small internal SRAM.
#define S3_ISOC_QTD_POOL_SLOTS                  6
#define S3_ISOC_QTD_SLOT_BYTES                  (XFER_LIST_LEN_ISOC * sizeof(usb_dwc_ll_dma_qtd_t))
static DRAM_ATTR uint8_t s_isoc_qtd_pool[S3_ISOC_QTD_POOL_SLOTS][S3_ISOC_QTD_SLOT_BYTES]
    __attribute__((aligned(USB_DWC_QTD_LIST_MEM_ALIGN)));
static bool s_isoc_qtd_pool_used[S3_ISOC_QTD_POOL_SLOTS];
static portMUX_TYPE s_isoc_qtd_pool_lock = portMUX_INITIALIZER_UNLOCKED;

// The default control pipe is also created after the application has built
// its model state. A tiny heap allocation can still fail then because DWC
// requires every qTD list to start on a 512-byte boundary. Keep the two
// double-buffered control lists at link time just like the isochronous lists;
// this makes unplug/replug independent of the current heap shape.
#define S3_CTRL_QTD_POOL_SLOTS                  NUM_BUFFERS
#define S3_CTRL_QTD_SLOT_BYTES                  USB_DWC_QTD_LIST_MEM_ALIGN
static DRAM_ATTR uint8_t s_ctrl_qtd_pool[S3_CTRL_QTD_POOL_SLOTS][S3_CTRL_QTD_SLOT_BYTES]
    __attribute__((aligned(USB_DWC_QTD_LIST_MEM_ALIGN)));
static bool s_ctrl_qtd_pool_used[S3_CTRL_QTD_POOL_SLOTS];
static portMUX_TYPE s_ctrl_qtd_pool_lock = portMUX_INITIALIZER_UNLOCKED;

static void *s3_isoc_qtd_pool_alloc(void)
{
    void *result = NULL;
    portENTER_CRITICAL(&s_isoc_qtd_pool_lock);
    for (int slot = 0; slot < S3_ISOC_QTD_POOL_SLOTS; ++slot) {
        if (!s_isoc_qtd_pool_used[slot]) {
            s_isoc_qtd_pool_used[slot] = true;
            result = s_isoc_qtd_pool[slot];
            break;
        }
    }
    portEXIT_CRITICAL(&s_isoc_qtd_pool_lock);
    if (result != NULL) {
        memset(result, 0, S3_ISOC_QTD_SLOT_BYTES);
    }
    return result;
}

static bool s3_isoc_qtd_pool_free(void *pointer)
{
    const uintptr_t base = (uintptr_t)s_isoc_qtd_pool;
    const uintptr_t end = base + sizeof(s_isoc_qtd_pool);
    const uintptr_t address = (uintptr_t)pointer;
    if (address < base || address >= end ||
            (address - base) % S3_ISOC_QTD_SLOT_BYTES != 0) {
        return false;
    }
    const size_t slot = (address - base) / S3_ISOC_QTD_SLOT_BYTES;
    portENTER_CRITICAL(&s_isoc_qtd_pool_lock);
    s_isoc_qtd_pool_used[slot] = false;
    portEXIT_CRITICAL(&s_isoc_qtd_pool_lock);
    return true;
}

static void *s3_ctrl_qtd_pool_alloc(void)
{
    void *result = NULL;
    portENTER_CRITICAL(&s_ctrl_qtd_pool_lock);
    for (int slot = 0; slot < S3_CTRL_QTD_POOL_SLOTS; ++slot) {
        if (!s_ctrl_qtd_pool_used[slot]) {
            s_ctrl_qtd_pool_used[slot] = true;
            result = s_ctrl_qtd_pool[slot];
            break;
        }
    }
    portEXIT_CRITICAL(&s_ctrl_qtd_pool_lock);
    if (result != NULL) {
        memset(result, 0, S3_CTRL_QTD_SLOT_BYTES);
    }
    return result;
}

static bool s3_ctrl_qtd_pool_free(void *pointer)
{
    const uintptr_t base = (uintptr_t)s_ctrl_qtd_pool;
    const uintptr_t end = base + sizeof(s_ctrl_qtd_pool);
    const uintptr_t address = (uintptr_t)pointer;
    if (address < base || address >= end ||
            (address - base) % S3_CTRL_QTD_SLOT_BYTES != 0) {
        return false;
    }
    const size_t slot = (address - base) / S3_CTRL_QTD_SLOT_BYTES;
    portENTER_CRITICAL(&s_ctrl_qtd_pool_lock);
    s_ctrl_qtd_pool_used[slot] = false;
    portEXIT_CRITICAL(&s_ctrl_qtd_pool_lock);
    return true;
}
#endif
 ...
 */
static hcd_pipe_event_t IRAM_ATTR _intr_hdlr_chan(pipe_t *pipe, usb_dwc_hal_chan_t *chan_obj, bool *yield)
{
 ...
 */
// Cycles spent in the host controller interrupt and how often it ran, for the
// pedalboard's audio window: the DSP stage on this core measures wall-clock
// cycles, so this is the part of its time the controller took.
DRAM_ATTR volatile uint32_t pedalboard_usb_isr_cycles;
DRAM_ATTR volatile uint32_t pedalboard_usb_isr_count;

// IRAM, along with the channel handler and the buffer parser it calls: the
// pedal's DSP shares the 16 KB instruction cache with a UI that boots from
// PSRAM, and an interrupt whose body has to be refetched through a thrashed
// cache stretches from a few microseconds to tens, at interrupt priority, on
// the core that runs stage A. Port events (_intr_hdlr_hprt) are rare and stay
// in flash.
static void IRAM_ATTR intr_hdlr_main(void *arg)
{
    port_t *port = (port_t *) arg;
    bool yield = false;
    const uint32_t isr_start = esp_cpu_get_cycle_count();
    ...
    pedalboard_usb_isr_cycles += esp_cpu_get_cycle_count() - isr_start;
    pedalboard_usb_isr_count += 1;
    if (yield) {
        portYIELD_FROM_ISR();
    }
}
...
{
#if !CONFIG_USB_HOST_HW_BUFFER_BIAS_PERIODIC_OUT
    ...
    // Duplex audio: preserve 384-byte periodic OUT and 64-byte control OUT,
    // then give capture the remaining *reported* FIFO depth. Do not assume
    // all 256 nominal hardware words are available for these FIFOs.
    port->fifo_config.nptx_fifo_lines = 16;
    port->fifo_config.ptx_fifo_lines = 96;
    port->fifo_config.rx_fifo_lines = fifo_size_lines > 112 ? fifo_size_lines - 112 : 0;
...
#if CONFIG_IDF_TARGET_ESP32S3
// The root port objects are allocated when usb_host_install() runs, which on
// this board is after the A2-Full graph has taken 213 KB of the 225 KB of
// internal SRAM. The heap is then 7 KB in fragments of 1.5 KB, so port_t, the
// HAL context and the 512-byte-aligned frame list -- none of which the heap can
// still place -- made hcd_port_init() return ESP_ERR_NO_MEM, and with it the
// whole USB host install. The pedal came up with a model, a panel and no audio
// device at all.
//
// The S3 has exactly one OTG peripheral, so one set at link time is the entire
// requirement, and it exists before anything can fragment around it. This is
// the same reservation the isochronous and control qTD lists above already use.
static DRAM_ATTR port_t s_root_port;
static DRAM_ATTR usb_dwc_hal_context_t s_root_port_hal;
static DRAM_ATTR uint32_t s_root_port_frame_list[FRAME_LIST_LEN]
    __attribute__((aligned(USB_DWC_FRAME_LIST_MEM_ALIGN)));
static bool s_root_port_used;
static portMUX_TYPE s_root_port_lock = portMUX_INITIALIZER_UNLOCKED;

static port_t *s3_root_port_claim(void)
{
    port_t *claimed = NULL;
    portENTER_CRITICAL(&s_root_port_lock);
    if (!s_root_port_used) {
        s_root_port_used = true;
        claimed = &s_root_port;
    }
    portEXIT_CRITICAL(&s_root_port_lock);
    if (claimed != NULL) {
        memset(claimed, 0, sizeof(*claimed));
        memset(&s_root_port_hal, 0, sizeof(s_root_port_hal));
        memset(s_root_port_frame_list, 0, sizeof(s_root_port_frame_list));
    }
    return claimed;
}

static bool s3_root_port_release(const port_t *port)
{
    if (port != &s_root_port) {
        return false;
    }
    portENTER_CRITICAL(&s_root_port_lock);
    s_root_port_used = false;
    portEXIT_CRITICAL(&s_root_port_lock);
    return true;
}
#endif
...
{
#if CONFIG_IDF_TARGET_ESP32S3
    port_t *port = s3_root_port_claim();
    usb_dwc_hal_context_t *hal = port != NULL ? &s_root_port_hal : NULL;
    void *frame_list = port != NULL ? s_root_port_frame_list : NULL;
#else
...
#if CONFIG_IDF_TARGET_ESP32S3
        (void)s3_root_port_release(port);
#else
        free(port);
        free(hal);
        free(frame_list);
...
esp_err_t hcd_port_init(int port_number, const hcd_port_config_t *port_config, hcd_port_handle_t *port_hdl)
{
    ...
    *port_hdl = (hcd_port_handle_t)port_obj;
    ESP_LOGI(HCD_DWC_TAG, "FIFO usable=%u words: RX=%" PRIu32 ", PTX=%" PRIu32 ", NPTX=%" PRIu32,
             (unsigned)port_obj->hal->constant_config.fifo_size,
             port_obj->fifo_config.rx_fifo_lines,
             port_obj->fifo_config.ptx_fifo_lines,
             port_obj->fifo_config.nptx_fifo_lines);
    extern void pedalboard_usb_fifo_configured(unsigned input, unsigned output) __attribute__((weak));
    if (pedalboard_usb_fifo_configured) {
        pedalboard_usb_fifo_configured((port_obj->fifo_config.rx_fifo_lines - 2) * 4,
                                      port_obj->fifo_config.ptx_fifo_lines * 4);
    }
...
static dma_buffer_block_t *buffer_block_alloc(usb_transfer_type_t type)
{
    ...
    if (buffer == NULL) {
        ESP_LOGE(HCD_DWC_TAG, "DMA buffer metadata allocation failed: internal=%u largest=%u",
    ...
    void *xfer_desc_list = NULL;
#if CONFIG_IDF_TARGET_ESP32S3
    if (type == USB_TRANSFER_TYPE_ISOCHRONOUS) {
        xfer_desc_list = s3_isoc_qtd_pool_alloc();
    } else if (type == USB_TRANSFER_TYPE_CTRL) {
        xfer_desc_list = s3_ctrl_qtd_pool_alloc();
    } else
#endif
    {
        xfer_desc_list = heap_caps_aligned_calloc(
            USB_DWC_QTD_LIST_MEM_ALIGN,
            desc_list_len * sizeof(usb_dwc_ll_dma_qtd_t), 1,
            XFER_DESC_LIST_CAPS);
    }

    if ((xfer_desc_list == NULL) || ((uintptr_t)xfer_desc_list & (USB_DWC_QTD_LIST_MEM_ALIGN - 1))) {
        ESP_LOGE(HCD_DWC_TAG,
                 "DMA descriptor allocation failed: type=%d ptr=%p internal=%u largest=%u",
                 type, xfer_desc_list,
                 (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
                 (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL));
        free(buffer);
#if CONFIG_IDF_TARGET_ESP32S3
        if (!s3_isoc_qtd_pool_free(xfer_desc_list) &&
                !s3_ctrl_qtd_pool_free(xfer_desc_list))
#endif
        {
...
#if CONFIG_IDF_TARGET_ESP32S3
    if (!s3_isoc_qtd_pool_free(buffer->xfer_desc_list) &&
            !s3_ctrl_qtd_pool_free(buffer->xfer_desc_list))
#endif
    {
...
esp_err_t hcd_pipe_alloc(hcd_port_handle_t port_hdl, const hcd_pipe_config_t *pipe_config, hcd_pipe_handle_t *pipe_hdl)
{
    ...
    if (pipe == NULL || chan_obj == NULL) {
        ESP_LOGE(HCD_DWC_TAG,
                 "pipe metadata allocation failed: pipe=%p channel=%p internal=%u largest=%u",
                 pipe, chan_obj,
                 (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
                 (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL));
        ret = ESP_ERR_NO_MEM;
    ...
    for (int i = 0; i < NUM_BUFFERS; i++) {
        ...
        if (buffers[i] == NULL) {
            ESP_LOGE(HCD_DWC_TAG, "pipe DMA buffer %d allocation failed for type=%d", i, type);
            ret = ESP_ERR_NO_MEM;
            goto err;
        }
    }
...
static inline void IRAM_ATTR _buffer_fill_isoc(dma_buffer_block_t *buffer, usb_transfer_t *transfer, bool is_in, int mps, int interval, int start_idx)
{
    ...
    int bytes_filled = 0;
    // Periodic DMA walks the complete 64-slot list. Keep every unscheduled slot
    // inert before placing this transfer's descriptors into the ring.
    memset(buffer->xfer_desc_list, 0,
           XFER_LIST_LEN_ISOC * sizeof(usb_dwc_ll_dma_qtd_t));
...
// Optional bounded diagnostic: retain the first raw DMA error per direction.
extern void pedalboard_usb_isoc_status(bool input, unsigned status) __attribute__((weak));
...
static inline void _buffer_parse_isoc(dma_buffer_block_t *buffer, bool is_in)
{
    ...
    for (int pkt_idx = 0; pkt_idx < transfer->num_isoc_packets; pkt_idx++) {
        ...
        if (desc_status && pedalboard_usb_isoc_status) {
            pedalboard_usb_isoc_status(is_in, (unsigned)desc_status);
        }
...
static void IRAM_ATTR _buffer_parse(pipe_t *pipe)
{
...
esp_err_t IRAM_ATTR hcd_urb_enqueue(hcd_pipe_handle_t pipe_hdl, urb_t *urb)
{
...
urb_t *IRAM_ATTR hcd_urb_dequeue(hcd_pipe_handle_t pipe_hdl)
{
    pipe_t *pipe = (pipe_t *)pipe_hdl;
    urb_t *urb;
