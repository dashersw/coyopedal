...
static esp_err_t dev_tree_node_new(ext_hub_handle_t parent, uint8_t port_num, usb_speed_t speed)
{
    ...
    dev_tree_node_t *dev_tree_node = heap_caps_calloc(1, sizeof(dev_tree_node_t), USB_HOST_OBJ_CAPS);
...
esp_err_t hub_install(hub_config_t *hub_config, void **client_ret)
{
    ...
    hub_driver_t *hub_driver_obj = heap_caps_calloc(1, sizeof(hub_driver_t), USB_HOST_OBJ_CAPS);
...
bool IRAM_ATTR hub_root_is_suspended(void)
{
    HUB_DRIVER_ENTER_CRITICAL();
