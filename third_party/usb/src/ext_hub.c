                                   ...
                                   size_t hub_desc_len)
{
    ...
    usb_hub_descriptor_t *desc = heap_caps_malloc(hub_desc_len, USB_HOST_OBJ_CAPS);
    if (desc == NULL) {
...
static esp_err_t device_alloc(device_config_t *config, ext_hub_dev_t **ext_hub_dev)
{
    ...
    ext_hub_dev_t *hub_dev = heap_caps_calloc(1, sizeof(ext_hub_dev_t), USB_HOST_OBJ_CAPS);
...
static esp_err_t device_configure(ext_hub_dev_t *ext_hub_dev)
{
    ...
    ext_hub_dev->constant.ports = heap_caps_calloc(ext_hub_dev->constant.hub_desc->bNbrPorts, sizeof(ext_port_hdl_t), USB_HOST_OBJ_CAPS);
...
{
    esp_err_t ret;
    ext_hub_driver_t *ext_hub_drv = heap_caps_calloc(1, sizeof(ext_hub_driver_t), USB_HOST_OBJ_CAPS);
    if (ext_hub_drv == NULL) {
        ret = ESP_ERR_NO_MEM;
        goto err;
    }
