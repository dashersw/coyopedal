...
static esp_err_t port_new(void *port_cfg, void **port_hdl)
{
    ...
    ext_port_t *ext_port = heap_caps_calloc(1, sizeof(ext_port_t), USB_HOST_OBJ_CAPS);
    if (ext_port == NULL) {
...
esp_err_t ext_port_install(const ext_port_driver_config_t *config)
{
    ...
    ext_port_driver_t *ext_port_drv = heap_caps_calloc(1, sizeof(ext_port_driver_t), USB_HOST_OBJ_CAPS);
