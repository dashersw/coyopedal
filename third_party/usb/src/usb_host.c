...
typedef struct {
    ...
    struct {
        ...
        // False for the normal audio-host configuration.  Avoid entering the
        // FreeRTOS timer API on every 1 ms endpoint completion when automatic
        // suspend has never been configured.
        bool auto_suspend_configured;
        union {
            struct {
                uint32_t handling_events: 1;
...
// The four functions below run inside the HCD interrupt (pipe callback ->
// endpoint_callback -> _unblock_client, and the processing-request path); see
// the note on intr_hdlr_main in hcd_dwc.c for why they are in IRAM.
static bool IRAM_ATTR _unblock_client(client_t *client_obj, bool in_isr)
{
    bool yield;
...
static bool IRAM_ATTR _unblock_lib(bool in_isr)
{
    bool yield;
...
{
    if (!p_host_lib_obj->dynamic.auto_suspend_configured) {
        return;
    }
...
static void send_event_msg_to_clients(const usb_host_client_event_msg_t *event_msg, bool send_to_all, uint8_t opened_dev_addr)
{
    ...
    TAILQ_FOREACH(client_obj, &p_host_lib_obj->mux_protected.client_tailq, dynamic.tailq_entry) {
        ...
        if (xQueueSend(client_obj->constant.event_msg_queue, event_msg, 0) == pdTRUE) {
            HOST_ENTER_CRITICAL();
            client_obj->dynamic.num_event_msgs++;
...
static bool IRAM_ATTR proc_req_callback(usb_proc_req_source_t source, bool in_isr, void *arg)
{
    HOST_ENTER_CRITICAL_SAFE();
...
static bool IRAM_ATTR endpoint_callback(usbh_ep_handle_t ep_hdl, usbh_ep_event_t ep_event, void *user_arg, bool in_isr)
{
...
esp_err_t usb_host_install(const usb_host_config_t *config)
{
    ...
    esp_err_t ret;
    host_lib_t *host_lib_obj = heap_caps_calloc(1, sizeof(host_lib_t), USB_HOST_OBJ_CAPS);
...
esp_err_t usb_host_lib_set_auto_suspend(usb_host_lib_auto_suspend_tmr_t timer_type, size_t timer_interval_ms)
{
    ...
    if (timer_interval_ms == 0) {
        ...
        p_host_lib_obj->dynamic.auto_suspend_configured = false;
    ...
    p_host_lib_obj->dynamic.auto_suspend_configured = true;
...
static void IRAM_ATTR _handle_pending_ep(client_t *client_obj)
{
...
esp_err_t usb_host_client_register(const usb_host_client_config_t *client_config, usb_host_client_handle_t *client_hdl_ret)
{
    ...
    client_t *client_obj = heap_caps_calloc(1, sizeof(client_t), USB_HOST_OBJ_CAPS);
    SemaphoreHandle_t event_sem = xSemaphoreCreateBinary();
...
esp_err_t IRAM_ATTR usb_host_client_handle_events(usb_host_client_handle_t client_hdl, TickType_t timeout_ticks)
{
    // Check arguments and state
    HOST_CHECK(client_hdl != NULL, ESP_ERR_INVALID_ARG);
    HOST_CHECK(p_host_lib_obj != NULL, ESP_ERR_INVALID_STATE);
    ...
    while (1) {
        ...
        while (client_obj->dynamic.num_event_msgs > 0) {
...
esp_err_t usb_host_get_config_desc(usb_host_client_handle_t client_hdl, usb_device_handle_t dev_hdl, uint8_t bConfigurationValue, const usb_config_desc_t **config_desc_ret)
{
    ...
    usb_config_desc_t *config_desc = heap_caps_malloc(config_desc_full->wTotalLength, USB_HOST_OBJ_CAPS);
    if (config_desc == NULL) {
        ret = ESP_ERR_NO_MEM;
        goto exit;
    }
...
{
    ep_wrapper_t *ep_wrap = heap_caps_calloc(1, sizeof(ep_wrapper_t), USB_HOST_OBJ_CAPS);
    if (ep_wrap == NULL) {
        return ESP_ERR_NO_MEM;
    }
    esp_err_t ret;
...
{
    interface_t *intf_obj = heap_caps_calloc(1, sizeof(interface_t) + (sizeof(ep_wrapper_t *) * intf_desc->bNumEndpoints), USB_HOST_OBJ_CAPS);
    if (intf_obj == NULL) {
        return NULL;
    }
...
esp_err_t IRAM_ATTR usb_host_transfer_submit(usb_transfer_t *transfer)
{
    ...
    if (urb_obj->usb_host_cached_device == transfer->device_handle &&
            urb_obj->usb_host_cached_ep_address == transfer->bEndpointAddress &&
            urb_obj->usb_host_cached_ep != NULL &&
            urb_obj->usb_host_cached_ep_context != NULL) {
        ep_hdl = (usbh_ep_handle_t)urb_obj->usb_host_cached_ep;
        ep_wrap = (ep_wrapper_t *)urb_obj->usb_host_cached_ep_context;
    } else {
        ...
        assert(ep_wrap != NULL);
        urb_obj->usb_host_cached_device = transfer->device_handle;
        urb_obj->usb_host_cached_ep_address = transfer->bEndpointAddress;
        urb_obj->usb_host_cached_ep = ep_hdl;
        urb_obj->usb_host_cached_ep_context = ep_wrap;
    }
...
esp_err_t IRAM_ATTR usb_host_transfer_resubmit(usb_transfer_t *transfer)
{
    HOST_CHECK(transfer != NULL, ESP_ERR_INVALID_ARG);
    urb_t *urb_obj = __containerof(transfer, urb_t, transfer);
    if (urb_obj->usb_host_cached_device != transfer->device_handle ||
            ...
            urb_obj->usb_host_cached_ep == NULL ||
            urb_obj->usb_host_cached_ep_context == NULL ||
            hub_root_is_suspended()) {
        return usb_host_transfer_submit(transfer);
    }

    HOST_CHECK(!urb_obj->usb_host_inflight, ESP_ERR_NOT_FINISHED);
    usbh_ep_handle_t ep_hdl =
        (usbh_ep_handle_t)urb_obj->usb_host_cached_ep;
    ep_wrapper_t *ep_wrap =
        (ep_wrapper_t *)urb_obj->usb_host_cached_ep_context;
    urb_obj->usb_host_inflight = true;
    HOST_ENTER_CRITICAL();
    ep_wrap->dynamic.num_urb_inflight++;
    HOST_EXIT_CRITICAL();

    esp_err_t ret = usbh_ep_resubmit_urb(ep_hdl, urb_obj);
    if (ret == ESP_OK) {
        return ESP_OK;
    }
    HOST_ENTER_CRITICAL();
