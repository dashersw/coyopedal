...
/**
 * @brief Resubmit a previously validated transfer to the same endpoint
 *
 * Falls back to usb_host_transfer_submit() until the endpoint cache has been
 * populated. Intended for fixed-shape realtime isochronous streams whose
 * packet lengths remain within the endpoint contract established by the first
 * successful submission.
 */
esp_err_t usb_host_transfer_resubmit(usb_transfer_t *transfer);
